module.exports = function(RED) {
    ...
    function DMXout(n) {
        ...
        var nanoDmx = new DMX(this.port, current_universe_buffer);
        this.on("input",function(msg) {
            current_universe = msg.payload;
            nanoDmx.update(current_universe, this.DMX_offset);
            node.send(current_universe);               
        });
        this.on('close', function(done) {
            nanoDmx.close(function (err) {
                console.log('nanodmx closed', err);
                done();
            });
        });
    }
